...
def main():
    ...
    topics = ["/sensors/novatel/gps_a/bestgnsspos", "/sensors/novatel/gps_b/bestgnsspos", "/sensors/novatel/gps_a/gps", "/sensors/novatel/gps_b/gps"]
    # =====================================================
    ...
    start_time = db["/sensors/novatel/gps_a/gps"]["header_t"].min()
    end_time = start_time + 500

    gps_a_df = select_time_range(
        db["/sensors/novatel/gps_a/gps"], start_time, end_time
    )
    gps_b_df = select_time_range(
        db["/sensors/novatel/gps_b/gps"], start_time, end_time
    )
    ...
    for df, ax, title in [
        ...
        x, y, gdop = x[::skip], y[::skip], gdop[::skip]
        gdop_clipped = np.clip(gdop, None, gdop_max)
    ...
    # shared colorbar
    fig.colorbar(sc, label=f"GDOP (≤{gdop_max})",
                 ax=axes.ravel().tolist(),
                 orientation="vertical")
    plt.suptitle("GPS A vs B Trajectories")
    #plt.tight_layout()
    plt.show()
